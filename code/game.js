...
var actorChars = {
  "@": Player,
  "o": Candy, // A candy will wobble up and down
  "=": Bat, "|": Bat,
  "B": Basket
};
...
Basket.prototype.type = "basket";

// Bat is initialized based on the character, but otherwise has a
// size and position
function Bat(pos, ch) {
  this.pos = pos;
  this.size = new Vector(1.5, 0.65);
  if (ch == "=") {
    // Horizontal bat
...
Bat.prototype.act = function(step, level) {
...
Basket.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  // Handle bat by calling playerTouched
  if (obstacle)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // if the player touches bat and the player hasn't won
  // Player loses
  if (type == "bat" && this.status == null) {
    ...
    if (!this.actors.some(function(actor) {
           return actor.type == "candy";
         })) {
               // If there aren't any baskets left, player wins
           if (!this.actors.some(function(actor) {
                  return actor.type == "basket";
                })) {
             this.status = "won";
             this.finishDelay = 2;
           }
